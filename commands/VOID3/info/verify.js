...
module.exports = {
    ...
    async execute (interaction) {
        ...
        let returnEmbed = new Discord.EmbedBuilder()
            ...
            .addFields(
                { name: "Verification Code", value: "Ask a recruiter to send you a verification code. It will be DM'd to you from the bot." },
        ...
        const collector = response.createMessageComponentCollector({ componentType: Discord.ComponentType.Button, time: 60_000 });
        collector.on('collect', async i => {
            await interaction.followUp({ content: `Verification`, embeds: [returnEmbed.setTimestamp()], components: [buttonRow1], ephemeral: true }).catch(console.error);
            ...
            if (foundVerifCode) {
                ...
                try {
                    ...
                    try {
                            ...
                            if (!member.user.bot) {
                                ...
                                if (badName) {
                                    ...
                                    if (evaluateRolesStatus == 1) {
                                        const rolesToRemove = org_removalRoles.concat(
                                            roleMap.find(i => i.rank_name == 'Sentinels').id,
                                            roleMap.find(i => i.rank_name == 'Guest').id
                                        )
                                ...
                                if (isSentinel == true && badName != true && org_cit_rank != null) {
                                    if (evaluateRolesStatus == 1) {
                                        ...
                                        // console.log("CHANGING NICKNAME:".cyan,citizen_handle)
                                    }
                                    if (evaluateRolesStatus == 0) {
                        ...
                        const member_channel = guild.channels.cache.get(config[botIdent().activeBot.botName].roleupdate.members_channel_test)
                        ...
                        if (success) { modalResults[0].editReply({ content: 'Verification Completed, welcome Sentinel', ephemeral: true }) }
                    }
                    catch (e) {
                        console.log(e)
                        botLog(
        ...
        collector.on('end', collected => {
            if (collected.size === 0) {
                interaction.followUp({ content: 'No button was clicked in time.', components: [] });
            }
        });
        collector.on('error', error => {
            console.error('Collector error:'.yellow, error);
        });
